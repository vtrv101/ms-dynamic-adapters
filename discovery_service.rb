  ...
  class DiscoveryService
    ...
    class << self
      def execute_discovery_request(request,params='')
        ...
        begin
          response = RestClient.post(@discovery_service_url, message,
            { :content_type => "text/xml; charset=UTF-8",
              "SOAPAction" => "http://schemas.microsoft.com/crm/2007/CrmDiscoveryService/Execute" })
          Nokogiri::XML(response)
        rescue RestClient::Exception => ex
          warn "DiscoveryService: Error in #{request}: " + ex.inspect
          raise ex
        end
      end  
      ...
      def select_node_text(doc,node_name)
        doc.xpath("//ns:#{node_name}/text()",
          {'ns'=>'http://schemas.microsoft.com/crm/2007/CrmDiscoveryService'}).to_s.strip
      end
      ...
      def get_user_organization
        ...
        select_node_text(doc,'OrganizationName[1]')
      end  
      ...
      def retrieve_ticket(user_organization)
        ...
        crm_ticket = select_node_text(doc,'CrmTicket')
        crm_ticket_expires = DateTime.parse(select_node_text(doc,'ExpirationDate'))
        crm_service_url = select_node_text(doc,'CrmServiceUrl')
